...
class Server( tornado.web.RequestHandler):
    ...
    def ug2nug( self, task) :
        sentence = task['text']
        file_tmp = codecs.open("tmp", "w", "utf-8")
        file_tmp.write(sentence)
        file_tmp.close()
        subprocess.call("cat tmp | ./uygur2latin.pl > tmp.latin", shell=True)
        #dont with converting
        lines = codecs.open("tmp.latin", "rb", "utf-8").readlines()
        ...
        #create json format
        lines = [line.strip().replace(u"↵",". ").replace(",.",".") for line in lines]
        #print lines
