...
boost::python::list translate_single(boost::python::list& in)
{
  ...
  std::string line = boost::python::extract<std::string>(boost::python::object(in[0]));
  
  SentencesPtr miniBatch(new Sentences());
  miniBatch->push_back(SentencePtr(new Sentence(god_, 0, line)));
  ...
  //God *GodPtr = &god_;
  //std::shared_ptr<Histories> tmpHistories = TranslationTask(god_, miniBatch);
  Search& search = god_.GetSearch();
  std::shared_ptr<Histories> tmpHistories = search.Translate(*miniBatch);

  
  LOG(info)->info("Translated took {}", timer2.format(3, "%ws"));
  ...
  allHistories.Append(*tmpHistories);
  //allHistories.SortByLineNum();
  ...
  string str = ss.str();
  boost::python::list output;
  //boost::python::list abc;
  output.append(str);
  //debug
  //string str_time = timer2.format(3, "%ws");
  //output.append(str_time);

  //output.append("FUCK YOU");
...
boost::python::list translate_batch(boost::python::list& in)
{
  ...
  for (size_t i = 0; i < allHistories.size(); ++i) {
    ...
    //const History& history = allHistories[i];
...
BOOST_PYTHON_MODULE(libamunmt)
{
  ...
  boost::python::def("translate_single", translate_single);
